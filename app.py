...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://wahmbnnbtouvrn:a1691b9ddcf3d34b1a7fa119e871c78ea2683b3e32d9353949916122d2c57fcc@ec2-3-224-164-189.compute-1.amazonaws.com:5432/d1obaqbldcesu8"
...
def add_month():
    if request.content_type != "application/json":
        return jsonify("Error, you did it wrong bro")
    ...
    if existing_month_check is not None:
        return jsonify("Error, your trying to manage the calendar wrong")
...
@app.route('/month/multiple/add', methods= ["POST"])
def add_multiple_month():
    if request.content_type != "application/json":
        return jsonify("Error, you did it wrong bro")
...
def add_reminder():
    ...
    if existing_reminder_check is not None:
        return jsonify("You can olny have one Reminder per day")
...
def update_remider(month_id, date):
    if request.content_type != "application/json":
        return jsonify("Error, di doesn't work like that")
...
def delete_remider(month_id, date):
